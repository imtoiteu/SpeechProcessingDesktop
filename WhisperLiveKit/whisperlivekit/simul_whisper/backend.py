...
class SimulStreamingOnlineProcessor:
    ...
    def __init__(self, asr, logfile=sys.stderr):
        ...
        self.end = 0.0
        self.buffer = []
        self._session_cfg = self._build_session_cfg()
    ...
    def _build_session_cfg(self):
        """Build a per-session AlignAttConfig honoring the session's language.

        SimulStreaming's ``transcribe()`` is a no-op, so the SessionASRProxy's
        ``original_language`` swap (which only LocalAgreement consults) has no
        effect here. Instead we clone the shared cfg with the session's language
        so the per-session AlignAtt builds the correct tokenizer / SOT / language
        detection (and ``self.model.cfg.language`` reflects the operating mode).
        The shared model weights are untouched — only ``cfg.language`` differs.
        """
        cfg = self.asr.cfg
        # Only a SessionASRProxy carries _session_language; raw ASR -> no override.
        sess = getattr(self.asr, "_session_language", "__unset__")
        if sess == "__unset__":
            return cfg
        lang = sess if sess is not None else "auto"  # proxy stores None for "auto"
        if lang == cfg.language:
            return cfg
        return dataclasses.replace(cfg, language=lang)
